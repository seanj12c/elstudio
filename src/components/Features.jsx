...
const Features = () => {
  ...
  const openModal = (index, description) => {
    let imageSrc;
    switch (index) {
      case 0:
        imageSrc = box;
  ...
  return (
    <div id="features" className="relative w-full">
      <div className="relative h-screen px-4 md:px-20 lg:px-32 text-white">
        <div>
          <h1 className="text-center pt-32 text-2xl md:text-5xl font-bold">
            Check our <span className="text-primary">feature</span> section
          </h1>
          <p className="text-center text-xs pb-5">Select Feature</p>
        </div>
        <div className="grid gap-5 grid-cols-2 ">
          {featuresData.map((feature) => (
            <div
              ...
              onClick={() => openModal(feature.id - 1, feature.description)}
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-75">
          <div className=" mx-auto">
            <img
              src={selectedImage}
              alt=""
              className="w-full h-80 px-5 md:h-[600px] object-cover"
            />
            <div className="md:flex px-10 pt-6">
              <p className="text-xs text-justify md:w-2/3 text-white">
                {selectedDescription}
              </p>
            </div>
            <div className="flex md:justify-end justify-center pt-2 px-10">
              <button
